...
there are _so many_ tools out there. This README explains our specific tech
...
If you _really_ think you need to add a dependency, **check with the team
...
- **What it offers:** ShadCN _copies_ the raw component code directly into our
...
- _Public Read, Admin Write:_ Anyone can view an event, but only users with an
  `admin` role can create or update an event.
- _User-Owned Data:_ A user can read and write their own profile details, but
...
Please, **do not** change `compilerOptions.jsx`! Next.js _requires_
